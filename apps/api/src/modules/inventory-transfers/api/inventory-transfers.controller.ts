...
import {
  cancelInventoryTransferRequestSchema,
  type CancelInventoryTransferRequest,
...
export class InventoryTransfersController {
  constructor(
    ...
    private readonly cancelInventoryTransferService: CancelInventoryTransferService,
  ...
  async cancelInventoryTransfer(
    ...
    @Param(new ZodValidationPipe(inventoryTransferIdParamsSchema))
    params: InventoryTransferIdParams,
    @Body(new ZodValidationPipe(cancelInventoryTransferRequestSchema))
    request: CancelInventoryTransferRequest,
  ): ReturnType<CancelInventoryTransferService['cancelTransfer']> {
    const session = await this.authService.getAuthenticatedRouteSession(authorizationHeader);
    const now = new Date();
    const requestIntent = { params, body: request };
    ...
    const idempotency = await this.idempotencyService.begin({
      ...
      requestIntent,
      now,
      expiresAt: this.cancelInventoryTransferService.getIdempotencyExpiresAt(now),
    });
    ...
    try {
      const response = await this.cancelInventoryTransferService.cancelTransfer(
        params.transfer_id,
        request,
        session.tenantContextSession,
      );
