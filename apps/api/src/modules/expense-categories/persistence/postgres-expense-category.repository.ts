...
} from '../application/expense-category.store';
...
@Injectable()
export class PostgresExpenseCategoryRepository extends ExpenseCategoryStore {
  constructor(
  ...
  async listExpenseCategories(
    ...
    return result.rows.map(toExpenseCategoryRecord);
  }
  ...
  async findExpenseCategoryById(
    ...
    return row === undefined ? null : toExpenseCategoryRecord(row);
  }
  ...
  async createExpenseCategory(
    ...
    if (row === undefined) {
      throw new Error('expense category create did not return a row.');
    }

    return toExpenseCategoryRecord(row);
  }
  ...
  async findExpenseCategoryDeactivationBlockers(
    tenantId: string,
    categoryId: string,
    client: DatabaseQueryClient = this.database,
  ): Promise<readonly ExpenseCategoryDeactivationBlocker[]> {
    const blockers = new Set<ExpenseCategoryDeactivationBlocker>();

    if (await this.tableExists('expenses', client)) {
...
function toExpenseCategoryRecord(row: ExpenseCategoryRow): ExpenseCategoryRecord {
  return {
    id: row.id,
