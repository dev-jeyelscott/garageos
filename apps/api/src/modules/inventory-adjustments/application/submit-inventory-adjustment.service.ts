...
import { assertCanSubmitForApproval } from './inventory-adjustment-state-machine';
...
@Injectable()
export class SubmitInventoryAdjustmentService {
  constructor(
  ...
  async submit(
    ...
    return this.transactionRunner.runInTransaction(async (transaction) => {
      ...
      assertCanSubmitForApproval(locked.adjustment);

      const now = new Date();
      const updated = await this.inventoryAdjustmentStore.markAdjustmentPendingApproval(
      ...
      await this.inventoryAdjustmentStore.insertStatusEvent(
        {
          ...
          adjustmentId,
          fromStatus: INVENTORY_ADJUSTMENT_STATUSES.DRAFT,
          toStatus: INVENTORY_ADJUSTMENT_STATUSES.PENDING_APPROVAL,
          reason: null,
