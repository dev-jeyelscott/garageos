...
import {
  ...
  cancelInventoryAdjustmentRequestSchema,
  ...
  type CancelInventoryAdjustmentRequest,
...
export class InventoryAdjustmentsController {
  constructor(
    ...
    private readonly cancelInventoryAdjustmentService: CancelInventoryAdjustmentService,
  ...
  @Post(':adjustment_id/cancel')
  @HttpCode(200)
  async cancelInventoryAdjustment(
    @Headers('authorization') authorizationHeader: string | undefined,
    @Headers('idempotency-key') idempotencyKey: string | undefined,
    @Param('adjustment_id') adjustmentId: string,
    @Body(new ZodValidationPipe(cancelInventoryAdjustmentRequestSchema))
    request: CancelInventoryAdjustmentRequest,
  ): ReturnType<CancelInventoryAdjustmentService['cancel']> {
    const session = await this.authService.getAuthenticatedRouteSession(authorizationHeader);
    const now = new Date();
    ...
    const idempotency = await this.idempotencyService.begin({
      tenantId: session.tenantContextSession.actor.tenant_id,
      userId: session.tenantContextSession.actor.user_id,
      endpoint: 'POST /api/v1/inventory-adjustments/{adjustment_id}/cancel',
      idempotencyKey,
      requestIntent: { adjustment_id: adjustmentId, ...request },
      now,
      expiresAt: this.cancelInventoryAdjustmentService.getIdempotencyExpiresAt(now),
    });
    ...
    try {
      const response = await this.cancelInventoryAdjustmentService.cancel(
        adjustmentId,
        request,
        session.tenantContextSession,
      );
